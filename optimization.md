...
> [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md). It lists everything that is **not yet
...
- Vote privacy (values never leave the server pre-reveal) — implemented + unit-tested.
...
- Test suite (Vitest unit/components) — implemented.
...
| G5 | **No load testing** (k6/artillery) | Capacity claims are unproven; can't find the real ceiling | 🟠 Medium | Missing (Phase 1) |
...
| G19 | **No analytics** (product telemetry) | Can't measure rooms created, rounds played, drop-off | 🟢 Low (product) | Missing |
...
> lint → build → unit → non-blocking npm audit).

| Task | Details | Effort |
| --- | --- | --- |
| **T1 — Rate limiting (G1, G14)** | Add a Socket.io middleware with a token bucket: `room:create` ≤ 5/min/IP, `room:join` ≤ 30/min/IP, `vote:cast` ≤ 10/s/socket, plus a max concurrent sockets per IP (e.g. 20). Return `{ ok:false, error:'rate_limited' }` acks so the client shows a friendly message. Package: `@socket.io/rate-limiter` or ~60 lines of hand-rolled bucket in `server/index.mjs`. | 0.5–1 day |
...
| **T6 — CI pipeline (G6)** | GitHub Actions workflow: `npm ci` → `npm run lint` → `npm run build` → `npm test` → `npm audit --audit-level=high` (G16) as a non-blocking job. | 1 day |
...
| **T8 — Load testing (G5)** | Add a k6 script (`scripts/load/`) that opens N rooms × M sockets, casts votes, reveals, and asserts p95 broadcast latency < 200 ms. Document the ceiling you find in docs/ARCHITECTURE.md. | 1 day |
...
| **T11 — HTTP caching for static pages (G13)** | Add `Cache-Control` via Next `headers()`: `s-maxage=31536000, immutable` for hashed assets, and ISR (`revalidate`) or static for the static marketing pages if they ever become dynamic. | 0.5 day |
...
- [x] GitHub Actions CI (lint → build → unit → audit) — G6
